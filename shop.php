    ...
    <div class="container">
        <div class="row">
            ...
            if (isset($_GET['search'])) {
                $search = $_GET['search'];
                $select = "SELECT * FROM products WHERE name LIKE '%{$search}%' AND active != 2 ";
                $query = mysqli_query($conn, $select);

                $result = array();

                // Lưu kết quả tìm được vào mảng $search_result
                while ($row = mysqli_fetch_array($query)) {
                    $result[] = $row;
                }

                if (count($result) > 0) {
                    foreach ($result as $rowProducts) {
            ...
            ?>
                ...
                $sqlProduct = "SELECT * FROM products WHERE active != 2 ORDER BY sort_order ASC LIMIT $begin, $item";
                $queryProduct = mysqli_query($conn, $sqlProduct);
                while ($rowProducts = mysqli_fetch_array($queryProduct)) {
                    if ($rowProducts['active'] == 1 && $rowProducts['price'] != 0) {
                        ?>
